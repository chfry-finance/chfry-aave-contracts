...
task('full:freeze', 'Deploy lending pool for dev enviroment')
  ...
  .setAction(async ({ verify, pool }, DRE: HardhatRuntimeEnvironment) => {
    ...
    //注意，这里是poolAdmin，不是emergencyAdmin，这是两个地址
    const admin = await DRE.ethers.getSigner(await getGenesisPoolAdmin(poolConfig));
